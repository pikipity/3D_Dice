...
public class sketch_3D_Dice extends PApplet {
...
public void draw(){
  if(clickTag){
    float t=(System.currentTimeMillis()-pastTime)/1000.000f;//Calculate time for displacement
    speed=initSpeed+a*t;//calculate speed for phase Y
    displacement=Math.round((initSpeed*t+0.500f*a*Math.pow(t,2))*100)/100.000f;//calculate displacement
...
public void mouseClicked(){
  if(!clickTag){
    ...
    i=PApplet.parseInt(random(1,4));
    pastTime=System.currentTimeMillis();
    clickTag=true;
  }
}

public void dice(float x,float y,float z,float phaseX,float phaseY,float phaseZ){
  background(255);
  translate(x,y,z);
  rotateX(phaseX); 
  rotateY(phaseY);
...
public void drawFigure(){
  ...
  faceOne.fill(0);
  faceOne.ellipse(faceOne.width*0.5f,faceOne.height*0.5f,faceOne.height*0.5f,faceOne.height*0.5f);
  faceOne.endDraw();
  //Face 2 figure
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_3D_Dice" };
    if (passedArgs != null) {
